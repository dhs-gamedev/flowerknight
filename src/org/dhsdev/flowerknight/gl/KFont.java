...
import org.dhsdev.flowerknight.util.RenderUtil;
...
public class KFont {
     ...
     */
    public KFont(Font font, boolean antiAlias) {
    ...
    private void loadFont() {
        ...
        this.tex = new Texture(buffer, width, height);
    }
    ...
    public void drawText(String string, int x, int y, Color color, boolean lineBreakUp) {
        ...
        for (int i = 0; i < string.length(); i++) {
            ...
            System.out.println(drawX + "," + drawY + " : Img " + g.imageWidth + " , " + g.imageHeight);
            RenderUtil.drawRegion(tex.id(), new Rectangle(drawX, drawY, g.width, g.height), new Rectangle(g.x, g.y, g.width, g.height), new Dimension(g.imageWidth, g.imageHeight), color);
            drawX += g.width;
        }
